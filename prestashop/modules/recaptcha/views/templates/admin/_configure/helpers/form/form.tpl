...
{block name="input"}
            ...
            {*<div class="checkbox_all checkbox">
				{strip}
					<label for="{$id_checkbox|escape:'html':'UTF-8'}">                                
						...
						{l s='Select/Unselect all' mod='recaptcha'}
					</label>
				{/strip}
			</div>*}
            {foreach $input.values.query as $value}
   ...
   {elseif $input.name == 'ETS_RCF_GOOGLE_CAPTCHA_SECRET_KEY'}
        {$smarty.block.parent}
        <p class="help-block">
            <a target="_blank" href="{if isset($path) && $path }{$path nofilter}views/pdf/recaptcha_v2.pdf{/if}" title="{l s='How to get Site key and Secret key?' mod='recaptcha'}">{l s='How to get Site key and Secret key?' mod='recaptcha'}</a>
        </p>
    {elseif $input.name == 'ETS_RCF_GOOGLE_V3_CAPTCHA_SECRET_KEY'}
        {$smarty.block.parent}
        <p class="help-block">
            <a target="_blank" href="{if isset($path) && $path }{$path nofilter}views/pdf/recaptcha_v3.pdf{/if}" title="{l s='How to get Site key and Secret key?' mod='recaptcha'}">{l s='How to get Site key and Secret key?' mod='recaptcha'}</a>
        </p>
   {else}
    {$smarty.block.parent}
   {/if}
